...
const e621CollectImages: CollectImagesOptions = {
    callback: async (): Promise<MediaListItem[]> =>
        Array.from(document.querySelectorAll('article.thumbnail')).map(
            (elem) => {
    ...
    getPrevPageUrl: () =>
        document.querySelector('#paginator-prev')?.getAttribute('href') ??
        undefined,
    getNextPageUrl: () =>
        document.querySelector('#paginator-next')?.getAttribute('href') ??
        undefined,
};
